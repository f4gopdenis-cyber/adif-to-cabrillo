...
1. Lance `ADIF_to_Cabrillo_F4GOP.py` (nécessite Python avec Tkinter, inclus par défaut sous Windows).
2. Sélectionne ton fichier ADIF.
3. Renseigne les informations du concours (indicatif, puissance, catégorie, échange envoyé/reçu, etc.).
4. Choisis l'emplacement du fichier Cabrillo de sortie.
5. Clique sur **CONVERTIR ADIF → CABRILLO**.

## Compiler en exécutable Windows (.exe)

Un script `build_exe.bat` est fourni pour générer un exécutable autonome via [PyInstaller](https://pyinstaller.org/) :

1. Place `build_exe.bat` dans le même dossier que `ADIF_to_Cabrillo_F4GOP.py`.
2. Double-clique sur `build_exe.bat`.
3. Récupère `ADIF_to_Cabrillo_F4GOP.exe` dans le dossier `dist/` généré.
